...
const averageDailyTemperature = (weather) => {
  ...
  weather.forEach( week => {
    let totalPerWeek = 0;
    week.forEach( day => {
      totalPerWeek = totalPerWeek + day;
    });
    fullTotal = fullTotal + (totalPerWeek / 7);
  });
  avrgTotalTemp = fullTotal / weather.length;
  return avrgTotalTemp;
};
...
const lowestWeeklyAverage = (weather) => {
  ...
  weather.forEach( week => {
    ...
    fullTotal.push(totalPerWeek / 7);
  });
  return Math.min(...fullTotal);
};
...
const excel = (str) => {
  let row = [];
  let rowSum = [];
  let arrayToSplit = str.split('\n');
  arrayToSplit.forEach( string => {
    row.push(string.split(','));
  });
  row.forEach( row => {
    let sum = 0;
    row.forEach( num => {
